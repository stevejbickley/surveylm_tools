...
class ResponseIntervention(BaseModel):
    ...
    response_intervention_date: Optional[str] = None
    responsible_party: str
    targeted_relief_to_whom: Optional[str] = None
    description: str
...
class RecoveryFundActivation(BaseModel):
    ...
    recovery_fund_amount: Optional[str] = None
    recovery_fund_date: Optional[str] = None
    responsible_party: str
    targeted_relief_to_whom: Optional[str] = None
    description: str
    links: Optional[List[str]] = []
...
class ResponseIntervention(BaseModel):
    response_intervention_type: str
    response_intervention_date: Optional[str]
    responsible_party: str
    targeted_relief_to_whom: Optional[str]
    description: str
    links: Optional[List[str]]
    page_number: List[str]
...
class RecoveryFundActivation(BaseModel):
    recovery_fund_type: str
    recovery_fund_amount: Optional[str]
    recovery_fund_date: Optional[str]
    responsible_party: str
    targeted_relief_to_whom: Optional[str]
    description: str
    links: Optional[List[str]]
...
def extract_disaster_event_details_from_text(text_input, events_catalog, temperature_setting=0.7, max_tokens_setting=None, top_p_setting=1, presence_penalty_setting=0, n_setting=1, frequency_penalty_setting=0, logprobs_setting=False, model_setting="gpt-4o-mini", chain_of_thought=True, guiding_principles="Fact-driven and evidence-based, grounded in accurate data and reliable sources."):
    """
    Extracts information about disaster events from annual QLD disaster management reports.
    Parameters:
    - text_input (str): The raw text data to be processed.
    ...
    """
    system_prompt = f"""
    You are an AI assistant specialized in extracting structured disaster event information from text data. Your task is to analyze the provided text and extract details for each disaster event listed in the events catalog.
    ...
    **Instructions:**
    ...
    1. Extract the Following Information for Each Known Event from the Events Catalog:
        - **Event Name:** Name of the known disaster event.
        - **Event Dates:** Dates during which the known disaster event occurred.
        - **Response Interventions:** Details of each response intervention, including:
            - Type of intervention (e.g., Firefighting, Evacuation, Medical Assistance).
            - Date of intervention.
            - Responsible party (e.g., Local Fire Department, Red Cross, State Government).
            - Targeted relief recipients (e.g., Residents, Vulnerable Populations, Local Businesses).
            - Description or overview/outline of the intervention.
            - Links to supporting documents or resources.
            - Page numbers where the intervention is mentioned. Ensure that all extracted details are **directly sourced from the content on these pages**.
        - **Recovery Fund Activations:** Details of each recovery fund activation, including:
            - Type of fund (e.g., Financial Aid, Infrastructure Rebuilding, Emergency Grants).
            - Amount of fund as numeric values in Australian Dollars (e.g., 100000.0).
            - Activation date.
            - Responsible party (e.g., State Government, Federal Emergency Management Agency).
            - Targeted relief recipients (e.g., Affected Families, Small Businesses, Public Services).
            - Description of the fund activation.
            - Links to supporting documents or resources.
            - Page numbers where the fund activation is mentioned. Ensure that all extracted details are **directly sourced from the content on these pages**.
    
    2. **Cross-Referencing:**
        - For each extracted detail, **explicitly link it to the content on the specified page number**. Ensure that no information is inferred beyond what is explicitly stated in the text.
    
    3. **Output Format:**
        - Structure the extracted information into JSON format following the provided schema.
        - Ensure all fields align with the schema requirements.
        - For each extracted detail, verify that it is directly referenced in the text on the specified page numbers. If a particular piece of information is missing or cannot be directly sourced, set its value to `null` or an empty list, as appropriate.
        - **Strictly use only the content on the specified page numbers** for extracting the corresponding details. Do not reference or infer information from other sections or pages of the document.
       
    4. **Handling Multiple Mentions:**
        - If multiple response interventions or recovery fund activations are mentioned on the same page, create separate entries for each, ensuring each entry references the correct page number.
   
    5. **Handling Ambiguities:**
        - If a date is partially mentioned (e.g., only month and year), include it as is.
        - If certain details are unclear or not explicitly stated, set the corresponding field to `null` or an empty list.

    **Guiding principles:**
    - {guiding_principles}
    
    Begin extracting the data based on the instructions above.
    """
    # Dynamic user prompt to provide specific instructions
...
def collect_all_event_details(pdf_path, events_catalog, chunk_size=5, model_setting="gpt-4o-mini"):
    """
    ...
    for chunk_pages in chunk_pdf_text(pdf_path, chunk_size=chunk_size):
        ...
        response = extract_disaster_event_details_from_text(text_input=chunk_text, events_catalog=events_catalog, model_setting=model_setting)
...
events_database2 = collect_all_event_details(pdf_path="queensland-disaster-management-committee-annual-report-2023-2024.pdf", events_catalog=events_catalog, model_setting="gpt-4o")

#And write to excel
transform_events_dict_to_excel(events_database2, "queensland-disaster-management-committee-annual-report-2023-2024_events_database2.xlsx")
